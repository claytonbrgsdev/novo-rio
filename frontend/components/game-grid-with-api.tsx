      ...
      <div className="h-full p-5">
        ...
        <div className="bg-amber-100 h-full rounded-lg p-5 relative">
          ...
          <div className="grid grid-cols-3 gap-5 md:grid-cols-4">
            {/* Área para estabelecimentos da vila que serão carregados da API */}
            <div className="flex items-center justify-center h-40">
              <p className="text-amber-700">Carregando estabelecimentos...</p>
            </div>
          </div>
      ...
      <div className="h-full w-full p-0">
        <div className="bg-green-200 h-full w-full rounded-lg relative overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            ...
            {/* Grid de lotes - será preenchido com dados da API */}
            <div className="grid grid-cols-5 grid-rows-3 gap-0 relative z-10 w-full h-full">
              {terrains.map((terrain, index) => (
                <div
                  ...
                  onClick={() => onQuadrantClick(terrain.position)}
                >
                  <div className="text-center">
                    <div className="text-base sm:text-lg font-medium text-green-800">
                      {terrain.name || `Lote ${index + 1}`}
                    </div>
                    <div className="text-xs sm:text-sm text-green-700 mt-1">
                      {terrain.player_id ? `Jogador ${terrain.player_id.substring(0, 8)}` : "Disponível"}
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="bg-green-600 h-full rounded-lg p-3">
          <div className="grid grid-cols-5 grid-rows-3 gap-3 h-full">
            {quadrants.map((quadrant) => {
              const isActive = quadrant.position === activeQuadrant
              const isCenter = quadrant.position === "C2" // Assumindo que C2 é o centro
              ...
              return (
                <div
                  ...
                  onClick={() => onQuadrantClick(quadrant.position)}
                >
    ...
    <div className="h-full">
      <div className="bg-green-500 h-full rounded-lg">
        ...
        <div className="grid grid-cols-5 grid-rows-3 gap-3 h-[calc(100%-3rem)]">
          {activeQuadrantData.slots?.map((slot) => {
            ...
            return (
              ...
              >
                {slot.position === 7 && <UserCharacter size="micro" showTool={true} />}
