...
import { apiService, AUTH_STORAGE_KEYS } from "@/services/api"
import type { CharacterCustomization } from "@/types/player"
...
export default function CharacterCustomizer() {
  ...
  const handleSaveCharacter = async () => {
    ...
    try {
      ...
      // Se o usuário estiver autenticado, salvar também na API
      if (user) {
        // Garantir que temos um player_id válido
        let playerId = user.player_id;
        
        // Se não tiver player_id ou for 0, criar um player diretamente
        // Usando a rota /users/{user_id}/link-player que cria um player e o associa ao usuário
        if (!playerId) {
          try {
            // Não precisamos verificar players existentes. O endpoint link-player vai cuidar disso
            console.log("Criando novo player para o usuário", user.id);
            const playerResponse = await apiService.post<any>(`/users/${user.id}/link-player`, { player_name: characterName });
            playerId = playerResponse.id;
            console.log("Player criado ou obtido:", playerResponse);
            
            // Atualiza o user para evitar criar novamente em logins futuros
            setUser(prevUser => prevUser ? { ...prevUser, player_id: playerId } : prevUser);
            ...
            if (typeof window !== 'undefined') {
              ...
              if (storedUser) {
                ...
                updatedUser.player_id = playerId;
                localStorage.setItem("user", JSON.stringify(updatedUser));
              }
            }
          } catch (linkError: any) {
            console.error("Erro ao criar/obter player:", linkError);
            throw new Error(`Erro ao criar/obter player: ${linkError.message || 'Erro desconhecido'}`);
          }
        }
        ...
        try {
          // Vamos remover a verificação separada de personagens existentes que estava causando erros
          // e simplesmente tentar criar o personagem diretamente
          
          console.log(`Salvando personagem para player ${playerId}`);
          
          try {
            // Tentativa de salvar personagem - o backend deve lidar com a lógica de atualização vs. criação
            const response = await apiService.post<CharacterCustomization>(`/players/${playerId}/characters`, characterData);
            console.log("Resposta da API ao salvar personagem:", response);
            
            // Se chegou aqui, o personagem foi salvo com sucesso
            setDebugMessage("Personagem salvo com sucesso. Redirecionando...");
          } catch (saveError) {
            // Se o erro for de conflito (409), provavelmente o personagem já existe
            console.log("Erro ao salvar personagem (pode ser normal se já existe):", saveError);
            setDebugMessage("Personagem já existente ou erro ao salvar. Continuando...");
          }
          
          // Independentemente se criou novo ou já existia, atualizamos o estado do usuário
          // com o player_id que já temos confirmado
          setUser(prevUser => prevUser ? { ...prevUser, player_id: playerId } : prevUser);
          console.log("Estado do usuário atualizado com player_id:", playerId);
        } catch (apiError: any) {
          console.error("Erro detalhado ao salvar personagem:", apiError);
          throw new Error(`Erro ao salvar personagem: ${apiError.message || 'Erro desconhecido'}`)
        }
      }
      ...
      // Redirecionar para o jogo após salvar usando o Next.js router
      // que preserva o estado da autenticação
      setTimeout(() => {
        // Redirecionando diretamente para a página do jogo, bypassando a página de transição
        console.log('Redirecionando para /game; token=', localStorage.getItem(AUTH_STORAGE_KEYS.TOKEN));
        router.push('/game')
      }, 1000)
    } catch (error) {
