...
import { loadGameSaves, loadCharacterCustomizations } from "@/lib/supabase"
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        const { data: savesData } = await loadGameSaves(user.id)
        if (savesData) {
          setGameSaves(savesData)
        }

        // Carregar personagens
        const { data: charactersData } = await loadCharacterCustomizations(user.id)
        if (charactersData) {
