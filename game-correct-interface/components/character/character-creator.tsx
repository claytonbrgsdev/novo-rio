...
import { loadCharacterCustomizations, saveCharacterCustomization } from "@/lib/supabase"
import type { CharacterCustomization } from "@/lib/supabase"
...
export default function CharacterCreator() {
    ...
    const fetchCharacters = async () => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          setName(data[0].name)
          setHeadId(data[0].head_id)
          setBodyId(data[0].body_id)
          setToolId(data[0].tool_id)
        }
      } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await saveCharacterCustomization(character)
      if (error) throw error

      setMessage({ type: "success", text: "Personagem salvo com sucesso!" })

      // Recarregar personagens
      const { data } = await loadCharacterCustomizations(user.id)
      if (data) {
        setCharacters(data)
      }
    } catch (error: any) {
  ...
  const handleSelectCharacter = (character: CharacterCustomization) => {
    ...
    setHeadId(character.head_id)
    setBodyId(character.body_id)
    setToolId(character.tool_id)
  }
