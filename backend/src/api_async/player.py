...
from ..crud.player import create_player, get_players, get_player, update_player, delete_player
...
async def list_players_endpoint(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
async def update_player_endpoint(player_id: int, player_update: PlayerUpdate, db: Session = Depends(get_db)):
    ...
    player = await run_in_threadpool(update_player, db, player_id, player_update)
    if not player:
        raise HTTPException(status_code=404, detail="Player not found")
    return player
