...
import { saveGameData, loadGameSaves, deleteGameSave } from "@/lib/supabase"
...
export default function SaveGameModal({ isOpen, onClose, gameState, onSaveComplete }: SaveGameModalProps) {
  ...
  const handleSaveGame = async () => {
    ...
    try {
      ...
      const { error } = await saveGameData(saveData)
      if (error) throw error
  ...
  const handleOverwriteSave = async (saveId: string) => {
    if (!user) return
